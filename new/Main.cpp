...
void calcmain() {
	input::getconfig("/home/voiafbr/SigCup/Datas/data.config");
...
#pragma omp parallel for
	for (int i = 0; i < res2.size(); i++){
		...
		auto res3 = query::onequery(res2[i], dfsleft[threadnum], dfsdown[threadnum], dfsheap[threadnum], dfsstage[threadnum]);
		for (auto &i : res3)
			fprintf(f, "%s\n", query::traj[i].name.c_str());
		fclose(f);
#ifdef DEBUG
